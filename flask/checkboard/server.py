...
def one():
    return render_template("index2.html",x=8,y=8,width=8*60,height=8*60,c1="black",c2="red")  # Return the string 'Hello World!' as a response
...
def three(x,y):
    width=int(y)*50
    height=int(x)*60
    return render_template("index2.html",x=int(x),y=int(y),width=width,c1="black",c2="red",height=height)  # Return the string 'Hello World!' as a response

@app.route('/<x>/<y>/<c1>/<c2>')          # The "@" decorator associates this route with the function immediately following
def four(x,y,c1,c2):
    width=int(y)*60
    height=int(x)*60
    return render_template("index2.html",x=int(x),y=int(y),width=width,c1=c1,c2=c2,height=height)  # Return the string 'Hello World!' as a response
